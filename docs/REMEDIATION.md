...
| F-04 | Plaintext SMTP creds + DB dumps exposed | SMTP config is Settings-managed (`core_config`); password is AES-256-GCM encrypted at rest with `APP_KEY` (never plaintext in DB/dumps/SQLi) — key itself stays in `.env` only. No `*.sql` outside `database/migrations/`. `.htaccess` + vhosts deny `.sql`/`.env`/`config.php`/dumps. | `includes/security.php` `sm_encrypt_secret()`/`sm_decrypt_secret()`, `includes/functions.php` `sendEmail()`, `includes/controllers/actions.php` (`emailSettings`), `.htaccess`, `deploy/*` | `/config.php`,`/*.sql`,`/.env` → 403; `core_config.email_smtp_password` is ciphertext, not plaintext |
