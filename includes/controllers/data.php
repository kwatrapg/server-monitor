...
if($_SESSION['range_type'] == "manual") {
	...
	if ($route == "domains/manage") {
		if($_SESSION['asset'] != "domain-".$_GET['id']) App::resetRange();
	}
	if ($route == "ssl/manage") {
		if($_SESSION['asset'] != "ssl-".$_GET['id']) App::resetRange();
	}
}
...
$main_domains_unresolved = getTableFiltered("app_domains_incidents","status[!]","1","","","*","id","ASC");
$main_ssl_unresolved = getTableFiltered("app_ssl_incidents","status[!]","1","","","*","id","ASC");
...
foreach ($main_domains_unresolved as $key => $value) {
	if(!checkGroup(get_group("app_domains", $value['domainid']))) {
		unset($main_domains_unresolved[$key]);
	}
}

foreach ($main_ssl_unresolved as $key => $value) {
	if(!checkGroup(get_group("app_ssl", $value['sslid']))) {
		unset($main_ssl_unresolved[$key]);
	}
}
...
if ($route == "dashboard") {
	...
	$domains_count = countTableFiltered("app_domains","groupid",$liu_groups);
	$ssl_count = countTableFiltered("app_ssl","groupid",$liu_groups);
...
// DOMAINS
if ($route == "domains") {
	isAuthorized("viewDomains");

	$pageTitle = __("Domains");
}

if ($route == "domains/manage") {
	isAuthorized("viewDomains");
	$domain = getRowById("app_domains", $_GET['id']);
	checkGroupRedirect($domain['groupid']);
	$alerts = getTableFiltered("app_domains_alerts","domainid",$_GET['id'],"","","*","id","ASC");
	$incidents = getTableFiltered("app_domains_incidents","domainid",$_GET['id'],"","","*","id","DESC");
	$unresolved_incidents = getTableFiltered("app_domains_incidents","domainid",$_GET['id'],"status[!]","1","*","id","ASC");
	$detailed_log = $database->select("app_domains_history", "*", [ "domainid" => $_GET['id'], "ORDER" => ['id' => 'DESC'], "LIMIT" => 100 ]);
	...
	$latest = Domain::latestData($_GET['id']);

	$start = $_SESSION['range_start'];
	$end = $_SESSION['range_end'];

	$all_history = $database->select("app_domains_history","id", [ "AND" => ["domainid" => $_GET['id'], "timestamp[>=]" => $start, "timestamp[<=]" => $end ] ]);
	...
	if($all_history < $max_entries) {
		$history = $database->select("app_domains_history", "*", [ "AND" => ["domainid" => $_GET['id'], "timestamp[>=]" => $start,"timestamp[<=]" => $end ] ]);
	}
	else {
        if(empty($fetch_history)) {
            $history = [];
        } else {
            $history = $database->select("app_domains_history", "*", [ "id" => $fetch_history ] );
        }
	...
	if(!empty($history)) {
		...
		foreach($history as $item) {
			...
			$charts['performance'][$i]['date'] = $label;
			$charts['performance'][$i]['latency'] = $item['days_remaining'];
...
// SSL
if ($route == "ssl") {
	isAuthorized("viewSsl");

	$pageTitle = __("SSL Certificates");
}

if ($route == "ssl/manage") {
	isAuthorized("viewSsl");
	$sslcert = getRowById("app_ssl", $_GET['id']);
	checkGroupRedirect($sslcert['groupid']);
	$alerts = getTableFiltered("app_ssl_alerts","sslid",$_GET['id'],"","","*","id","ASC");
	$incidents = getTableFiltered("app_ssl_incidents","sslid",$_GET['id'],"","","*","id","DESC");
	$unresolved_incidents = getTableFiltered("app_ssl_incidents","sslid",$_GET['id'],"status[!]","1","*","id","ASC");
	$detailed_log = $database->select("app_ssl_history", "*", [ "sslid" => $_GET['id'], "ORDER" => ['id' => 'DESC'], "LIMIT" => 100 ]);
	...
	$latest = Ssl::latestData($_GET['id']);

	$start = $_SESSION['range_start'];
	$end = $_SESSION['range_end'];

	$all_history = $database->select("app_ssl_history","id", [ "AND" => ["sslid" => $_GET['id'], "timestamp[>=]" => $start, "timestamp[<=]" => $end ] ]);
	...
	if($all_history < $max_entries) {
		$history = $database->select("app_ssl_history", "*", [ "AND" => ["sslid" => $_GET['id'], "timestamp[>=]" => $start,"timestamp[<=]" => $end ] ]);
	}
	else {
        if(empty($fetch_history)) {
            $history = [];
        } else {
            $history = $database->select("app_ssl_history", "*", [ "id" => $fetch_history ] );
        }
