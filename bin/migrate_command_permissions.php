<?php
/**
 * One-time migration: grants the new custom-command permissions
 * (viewCommands, manageCommands) to existing roles, mirroring each role's
 * current Servers access level. Safe to re-run (idempotent).
 *
 * core_roles.perms is a PHP-serialized array, so this has to be done in PHP,
 * never with raw SQL string surgery on the column. See
 * bin/migrate_log_permissions.php for the identical pattern used for the
 * server-log-monitoring permissions.
 *
 ...
 *
 *   php bin/migrate_command_permissions.php            # apply
 *   php bin/migrate_command_permissions.php --status    # show what would change, no writes
 */
...
foreach ($roles as $role) {
    ...
    $add = [];
    if (in_array('viewServers', $perms, true)) $add[] = 'viewCommands';
    if (in_array('editServer', $perms, true))  $add[] = 'manageCommands';
...
echo "\n" . ($dryRun ? "Dry run - no changes written." : "Done. Refresh the app (no re-login needed) to see the Commands nav item.") . "\n";
